...
import { Users2, Search, Trash2, Eye, ShieldOff, ShieldCheck } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-extrabold text-gray-800 tracking-tight">Manage Users</h1>
          <p className="text-sm text-gray-400 mt-0.5">{allUsers.length} registered Users</p>
        </div>

        {/* Summary badges */}
        <div className="hidden sm:flex items-center gap-2">
          <span className="text-xs font-semibold px-3 py-1.5 rounded-full border bg-green-100 text-green-600 border-green-200">
            {allUsers.filter((u) => u.status === "Active").length} Active
          </span>
          <span className="text-xs font-semibold px-3 py-1.5 rounded-full border bg-red-50 text-red-500 border-red-200">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-xs uppercase tracking-wider text-gray-400 border-b border-gray-100">
                ...
                <th className="px-5 py-3 text-left font-semibold hidden sm:table-cell">Email</th>
                    ...
                    <div className="flex items-center justify-center gap-1.5">
                      <button
                        ...
                        className={`w-8 h-8 rounded-lg border flex items-center justify-center transition-all cursor-pointer
                          ${!u.isBlocked
                            ? "bg-yellow-50 hover:bg-yellow-100 border-yellow-100 text-yellow-500"
                            : "bg-green-50 hover:bg-green-100 border-green-100 text-green-500"
                          }`}
