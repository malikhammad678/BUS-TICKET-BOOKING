import { Bus, MapPin, Clock, Pencil, Trash2, Plus, Search } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 tracking-tight">Bus List</h1>
          <p className="text-sm text-gray-400 mt-0.5">{buses.length} buses in the system</p>
        </div>
        <button
          onClick={() => navigate("/owner/add-bus")}
          className="flex items-center gap-2 bg-primary text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:scale-[1.02] hover:shadow-md hover:shadow-primary/20 transition-all duration-200 cursor-pointer"
        >
          <Plus size={16} />
          Add Bus
        </button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-xs uppercase tracking-wider text-gray-400 border-b border-gray-100">
                ...
                <th className="px-5 py-3 text-left font-semibold hidden sm:table-cell">Schedule</th>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button
                        ...
                        className="w-8 h-8 rounded-lg bg-red-50 hover:bg-red-100 border border-red-100 flex items-center justify-center text-red-400 transition-all duration-150 cursor-pointer"
                        title="Delete"
                      >
                        <Trash2 size={14} />
                      </button>
